...
class CouponControllerTest {
    ...
    void testGetCouponsByStatus() throws Exception {
        // Given
        Pageable pageable = PageRequest.of(0, 10);
        CouponStatus status = CouponStatus.PENDING;
    ...
    @Test
    void testGetCouponsByType() throws Exception {
        // Given
        Pageable pageable = PageRequest.of(0, 10);
        DiscountType discountType = DiscountType.PERCENTAGE;

        CouponPolicyResponse mockPolicyResponse1 = new CouponPolicyResponse(1L, discountType, new BigDecimal("10"), new BigDecimal("500"), new BigDecimal("10000"));
        CouponPolicyResponse mockPolicyResponse2 = new CouponPolicyResponse(2L, discountType, new BigDecimal("15"), new BigDecimal("1000"), new BigDecimal("20000"));
        ...
        when(couponService.getCouponsByType(any(DiscountType.class), any(Pageable.class))).thenReturn(mockPage);

        // When & Then
        mockMvc.perform(get("/api/admin/coupons/type/{type}", discountType)
                        .param("page", "0")
                        .param("size", "10")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk()) // 응답 상태 코드 확인
                .andExpect(jsonPath("$.content[0].couponId").value(1L)) // 첫 번째 쿠폰 ID 확인
                .andExpect(jsonPath("$.content[0].couponName").value("Coupon 1")) // 첫 번째 쿠폰 이름 확인
                .andExpect(jsonPath("$.content[0].couponPolicyResponse.discountType").value("PERCENTAGE")) // 첫 번째 쿠폰 할인 타입 확인
                .andExpect(jsonPath("$.content[1].couponId").value(2L)) // 두 번째 쿠폰 ID 확인
                .andExpect(jsonPath("$.content[1].couponName").value("Coupon 2")) // 두 번째 쿠폰 이름 확인
                .andExpect(jsonPath("$.content[1].couponPolicyResponse.discountType").value("PERCENTAGE")); // 두 번째 쿠폰 할인 타입 확인

        verify(couponService, times(1)).getCouponsByType(any(DiscountType.class), any(Pageable.class)); // 서비스 호출 검증
    }
    ...
    void testChangeIssuedCoupon() throws Exception {
        ...
        doNothing().when(couponService).changeCouponStatus(anyLong(), eq(CouponStatus.ISSUED));
        ...
        verify(couponService, times(1)).changeCouponStatus(couponId, CouponStatus.ISSUED); // 서비스 호출 검증
    }
